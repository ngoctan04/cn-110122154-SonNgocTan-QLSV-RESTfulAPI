...
const UpdateUser = () => {
    ...
    // Fetch classes & majors for dropdowns
    useEffect(() => {
        const fetch = async () => {
            try {
        ...
        try {
            ...
            if (response.data.success && response.data.data) {
                ...
                const normalized = {
                    ...
                    className: u.classId?.className || u.className || '',
                    majorId: u.majorId?._id || '',
                    majorName: u.majorId?.majorName || u.majorName || ''
                };
                setUser(normalized);
            ...
            toast.error('Không thể tải thông tin sinh viên', { position: 'top-right' });
        } finally {
    ...
    return (
        <div className='addUser'>
            ...
            <form className='addUserForm' onSubmit={submitForm}>
                ...
                <div className='inputGroup'>                    <label htmlFor='mssv'>MSSV</label>
                    <input
                        type='text'
                        id="mssv"
                        onChange={inputHandler}
                        name='mssv'
                        placeholder='VD: 110122001'
                        value={user.mssv || ''}
                        className='form-control'
                    />
                </div>

                <div className='inputGroup'>                    <label htmlFor='phone'>Số điện thoại</label>
                    <input
                        type='tel'
                        id="phone"
                        onChange={inputHandler}
